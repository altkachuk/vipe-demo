...
# 1. Prepare dataset
# =========================
run_step "Prepare dataset" "bash \"$SCRIPTS_DIR/run_prepare_dataset.sh\" \"$DATASET_NAME\""

# =========================
# 2. COLMAP
# =========================
if [ "$SKIP_COLMAP" = false ]; then
  run_step "COLMAP reconstruction" "bash \"$SCRIPTS_DIR/run_colmap.sh\" \"$DATASET_NAME\""
else
...
if [ "$SKIP_VIPE" = false ]; then
  run_step "ViPE processing" "bash \"$SCRIPTS_DIR/run_vipe.sh\" \"$DATASET_NAME\""
else
